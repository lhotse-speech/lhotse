...
class BatchMemoryLoader:
    ...
    def __call__(self, cuts: CutSet) -> CutSet:
        ...
        for url in urls:
            ...
            if output_fmt is None:
                ...
                # Archive, path will link to file in archive
                file_path = parts[1].split("/", 1)

                # Obj name is archive name
                obj_name = file_path[0]

                # Get object
                obj = bck.object(obj_name)

                # Archpath is archived file name
                archpath = file_path[1]
    ...
    def get_output_format(self, path: str) -> Optional[str]:
        ...
        Returns:
            str (optional): Output type (file extension) if path contains supported archive type, else None
        """
