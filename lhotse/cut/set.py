...
class CutSet(Serializable, AlgorithmMixin):
    ...
    def contains_only_mono_cuts(self) -> bool:
        return all(isinstance(cut, MonoCut) for cut in self)

    def has_one_supervision_per_cut(self) -> bool:
        return all(len(cut.supervisions) == 1 for cut in self)
    ...
    def _convert_cuts_info_to_hf(self) -> Tuple[Dict[str, Any], Dict[str, Any]]:
        ...
        cut_info = {
            "id": [cut.id for cut in self],
            "audio": [cut.recording.sources[0].source for cut in self],
            "duration": [cut.duration for cut in self],
            "num_channels": [len(cut.recording.channel_ids) for cut in self],
        }
    ...
    def _convert_supervisions_info_to_hf(
        self,
        has_speaker: bool,
        has_language: bool,
        alignment_types: Union[List[str], Set[str]],
        ...
        :param alignment_types: A list|set of alignment types.
    ...
    def to_huggingface_dataset(self):
        ...
        cut_info, cut_info_description = self._convert_cuts_info_to_hf()
        sup_dicts, sup_dicts_info = self._convert_supervisions_info_to_hf(
            has_speaker,
            has_language,
            alignment_types,
            exclude_attributes={"start", "end", "channel"}
            if self.has_one_supervision_per_cut()
            else None,
        )
