...
class K2AlignmentRestrictedTrasducerDataset(torch.utils.data.Dataset):
    """
    The PyTorch Dataset for the AR-RNNT task using k2 library.
    ...
    The key difference between this dataset and the K2SpeechRecognitionDataset is that
    we expect each item to contain word-level alignments, and the dataset class
    is initialized with a BPE model. When fetching a batch, we convert the word-level
    alignments to BPE-level alignments and return them in the form of valid
    time ranges per BPE token. In icefall, this can then be used to define prune_ranges
    for RNN-T training (instead of estimating prune ranges from a first-pass simple
    joiner as is done in pruned_transducer).
    ...
    .. code-block::

        {
            ...
            'valid_ranges': List of tensors of shape (S, 2) where S is the number of
                            output tokens.
        }
