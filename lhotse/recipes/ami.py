...
def download_ami(
    ...
    annotations_path = target_dir / annotations_name
        ...
        for file in archive.namelist():
            if file.startswith('words/') and file[-1] != '/':
                ...
                if (meet_id,x) not in global_spk_id:
                    logging.warning(f'No speaker {meet_id}.{x} found! Skipping annotation.')
                    continue
        ...
        for key in annotations:
            ...
            for seg in list(annotations[key])[1:]:
                if seg.begin_time - cur_seg.end_time <= max_pause:
                    cur_seg._replace(text=f"{cur_seg.text} {seg.text}", end_time=seg.end_time)
                else:
        ...
        for source in recording.sources:
            ...
            for seg_idx, seg_info in enumerate(annotation):
                ...
                if seg_info.end_time > recording.duration:
                    logging.warning(f'Segment {recording.id}-{channel}-{seg_idx} exceeds '
...
def prepare_ami(
    ...
    annotations = parse_ami_annotations(data_dir / 'annotations.zip', max_pause=max_pause)
    ...
    for part in ['train', 'dev', 'test']:
        ...
        supervision_part = supervision.filter(lambda x: x.recording_id in dataset_parts[part])
