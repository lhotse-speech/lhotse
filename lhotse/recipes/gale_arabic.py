...
# Test recordings (from https://github.com/kaldi-asr/kaldi/blob/master/egs/gale_arabic/s5d/local/test/test_p2)
TEST = [
...
def prepare_gale_arabic(
    ...
    """
    Prepare manifests for GALE Phase 2 Arabic Broadcast Conversation corpus.

    :param audio_dirs: List of paths to ``LDC2013S02``, ``LDC2013S07``, and ``LDC2014S07``.
    :param transcripts_dirs: List of paths to ``LDC2013T17``, ``LDC2013T04``, and ``LDC2014T17``.
    ...
    # Some of the audio is wav while others are flac
    audio_paths = chain.from_iterable(
        [
...
def parse_transcripts(transcript_paths: List[Path]) -> List[SupervisionSegment]:
    ...
    for file in transcript_paths:
        with open(file, 'r') as f:
            # skip first 3 lines since they contain headers
            for _ in range(3):
                next(f)
