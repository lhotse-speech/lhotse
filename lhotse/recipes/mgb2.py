...
def prepare_mgb2(
    ...
    if output_dir is not None:
        ...
        manifests = read_manifests_if_cached(
            ...
            prefix="mgb2", suffix="jsonl.gz", lazy=True,
        )
    ...
    for part in dataset_parts:
        ...
        if part == "test" or part == "dev":
            ...
            with open(corpus_dir / part / "wav.scp", "r") as f_in, open(
                    output_dir / part / "wav.scp", "w") as f_out:
                for line in f_in:
                    f_out.write(line.replace(
                        "wav/", f"{corpus_dir}/{part}/wav/"))
                    f_out.write("\n")
            ...
            if part == "test":
                assert len(
                    supervisions) == 5365, f"Expected 5365 supervisions for test, found {len(supervisions)}"
            elif part == "dev":
                assert len(
                    supervisions) == 5002, f"Expected 5002 supervisions for dev, found {len(supervisions)}"
        elif part == "train":
        ...
        recordings.to_file(
            (output_dir / f"mgb2_recordings_{part}.jsonl.gz"))
        supervisions.to_file(
            (output_dir / f"mgb2_supervisions_{part}.jsonl.gz"))
