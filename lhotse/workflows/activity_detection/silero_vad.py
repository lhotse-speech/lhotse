...
def _get_activity_maker(sampling_rate: int):
...
class _SileroVAD(ActivityDetector, name="_silero_vad"):
    ...
    def __init__(
        ...
        *,
        ...
        if self.__class__ is _SileroVAD:  # pragma: no cover
            msg = f"{self.__class__.__name__} is an abstract class and cannot be instantiated directly."
            raise TypeError(msg)
        ...
        self._to_activity = _get_activity_maker(sampling_rate)
    ...
    @classmethod
    def hard_reset(cls):  # pragma: no cover
