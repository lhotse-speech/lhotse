...
def test_window_durations_are_uniform():
    ...
    windows = list(cut.cut_into_windows_balanced(min_duration=30, max_duration=40, overlap=1.0))
...
def test_chosen_duration_maximises_last_chunk():
    ...
    windows = list(cut.cut_into_windows_balanced(min_duration=min_d, max_duration=max_d, overlap=overlap))
    ...
    for d in range(min_d, max_d + 1):
        assert chosen_last >= last_chunk_len(d) - 1e-9, (
...
def test_cutset_balanced_produces_same_result_as_single_cut():
    ...
    cs_windows = list(cutset.cut_into_windows_balanced(min_duration=30, max_duration=40, overlap=1.0))
    direct_windows = list(cut.cut_into_windows_balanced(min_duration=30, max_duration=40, overlap=1.0))
...
def test_cutset_short_cuts_pass_through():
    ...
    windows = list(cutset.cut_into_windows_balanced(min_duration=30, max_duration=40, overlap=1.0))
